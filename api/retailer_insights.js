...
const INSIGHT_KEY = 'latest_retailer_insight';
...
async function generateInsight() {
  ...
  try {
    const query = `SELECT source_name, title, brand, title_sentiment_category FROM \`isentropic-keep-450218-e5.Brand.retailers_tb_sentiment\` WHERE DATE(published_at) = (SELECT DATE(MAX(published_at)) FROM \`isentropic-keep-450218-e5.Brand.retailers_tb_sentiment\`) ORDER BY published_at DESC`;
    ...
    if (!rows || rows.length === 0) {
      ...
      const prompt = `You are an expert data news analyst. Here is a JSON array of recent articles about Retailers. Each object contains 'source_name', 'title', 'brand' (retailer), and 'title_sentiment_category' (e.g., 'Positive', 'Negative', 'Neutral').

      Your task is to provide a concise, insightful, and actionable summary for a daily report. Focus on:
      1.  **Top Brands (Retailers) by Coverage:** Identify which brands received the most articles.
      2.  **Sentiment Analysis (Highlighting Negative):** For each mentioned brand, summarize their overall sentiment. **Explicitly highlight if a retailer has a notable amount of negative sentiment and provide a brief context if possible (e.g., "X had significant negative sentiment due to Y").**
      3.  **Top Publishing Sources:** Identify which sources published the most articles related to brands.
